...
def test_render_escapes_and_bolds():
    p = Post(intro="A & B", themes=[Theme(title="<x> fast", body="<b>ody</b> & more")])
    out = render_html(p)
...
def test_render_omits_empty_themes_and_fixes():
    out = render_html(Post(intro="i", themes=[Theme(title="T", body="B")]))
    assert "🐞" not in out
    assert "Исправления" not in out


def test_render_includes_fixes_as_bullets():
    out = render_html(Post(intro="i", themes=[], fixes=["правка один", "правка два"]))
...
def test_finalize_publish_adds_number_and_footer():
    ...
    base = render_html(Post(intro="i", themes=[Theme(title="T", body="B")]))
